...
(async () => {
  try {
    ...
    for (const r of ["jobs","tasks","reports"]) {
      try { await comm.get(r); } catch {}
    }
    document.dispatchEvent(new CustomEvent('s1ui:ready', { detail: { module: 'dashboard-v2' } }));
  } catch (e) { console.warn('[dashboard-v2] init error', e); }
})();
